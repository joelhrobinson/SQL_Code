...
--  CREATE TRIGGER 
--	can be BEFORE UPDATE, AFTER UPDATE or INSTEAD OF UPDATE 

CREATE TRIGGER updatetrigger ON Locations
--		alter  TRIGGER updatetrigger ON Locations
INSTEAD OF UPDATE 
AS 
BEGIN
	select 'INSTEAD OF UPDATE: TRIGGER: Someone tried to update Locations table' as [message]
END
